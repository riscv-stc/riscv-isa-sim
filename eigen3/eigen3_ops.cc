...
int CustomInsns::meconv_mm(half *rs1, half *rs2, half *rd, struct ConvShapeStride *ss)
{
    ...
    #pragma omp parallel for default(shared) private(j,ii,jj,kk)
    for (i = 0; i < h; i++) {
        for (j = 0; j < w; j++) {
...
{  
    ...
    /*calculate the kernel shape*/
    Bfloat16 *rs2_tmp = (Bfloat16 *)malloc(kh * kw * in_c * out_c * sizeof(Bfloat16));
    Map_Bfloat16 rs2_matrix(rs2_tmp, kh * kw * in_c, out_c, DynStride(out_c, 1));
    ...
    int i, j, k, ii, jj, kk, index_cin, counter;
    #pragma omp parallel for default(shared) private(j,ii,jj,kk,start,rs1_start)
    for (i = 0; i < h; i++) {
        for (j = 0; j < w; j++) {
...
int CustomInsns::meconv_mm(float32_t *rs1, float32_t *rs2, float32_t *rd, struct ConvShapeStride *ss)
{
    ...
    /*calculate the kernel shape*/
    float32_t *rs2_tmp = (float32_t *)malloc(kh * kw * in_c * out_c * sizeof(float32_t));
    Map_float32_t rs2_matrix(rs2_tmp, kh * kw * in_c, out_c, DynStride(out_c, 1));
...
int CustomInsns::meconv_mm(int8_t *rs1, int8_t *rs2, half *rd, struct ConvShapeStride *ss, half *deq_addr)
{
    ...
    /*calculate the kernel shape*/
    int8_t *rs2_tmp = (int8_t *)malloc(kh * kw * in_c * out_c * sizeof(int8_t));
    Map_int8_t rs2_matrix(rs2_tmp, kh * kw * in_c, out_c, DynStride(out_c, 1));
