...
#define check_traps_vexxx_mm(etype) ({ \
        ...
        int rs1_size = (STRIDE_RS1 ? STRIDE_RS1 : SHAPE1_COLUMN) * SHAPE1_ROW * sizeof(etype); \
        int rs2_size = (STRIDE_RS2 ? STRIDE_RS2 : SHAPE1_COLUMN) * SHAPE1_ROW * sizeof(etype); \
        int rd_size = (STRIDE_RD ? STRIDE_RD : SHAPE1_COLUMN) * SHAPE1_ROW  * sizeof(etype); \
...
#define check_traps_vexxx_mm_reduce_all(etype) ({ \
        ...
        int rs2_size = (STRIDE_RS2 ? STRIDE_RS2 : (SHAPE1_COLUMN * sizeof(etype))) * SHAPE1_ROW; \
        check_cust_access(RS1, rs1_size); \
        check_cust_access(RS2, rs2_size); \
    })
...
#define check_traps_vexxx_m_convert(dtype, stype) ({ \
        ...
        check_cust_invalid_shape(SHAPE1_COLUMN, SHAPE1_ROW); \
        int rs1_size = (STRIDE_RS1 ? STRIDE_RS1 : (SHAPE1_COLUMN * sizeof(stype))) * SHAPE1_ROW; \
        int rd_size = (STRIDE_RD ? STRIDE_RD : (SHAPE1_COLUMN * sizeof(dtype))) * SHAPE1_ROW; \
        check_cust_access(RS1, rs1_size); \
        check_cust_access(RD, rd_size); \
  })
...
#define check_traps_memul_mm(rs1_type, rs2_type, out_type) ({ \
        ...
        int rs1_size = (BC_STRIDE_RS1 ? BC_STRIDE_RS1 : BC_SHAPE1_COLUMN) * sizeof(rs1_type) * BC_SHAPE1_ROW; \
        int rs2_size = (BC_STRIDE_RS2 ? BC_STRIDE_RS2 : BC_SHAPE2_COLUMN) * sizeof(rs2_type) * BC_SHAPE2_ROW; \
        int rd_size = (BC_STRIDE_RD ? BC_STRIDE_RD : BC_SHAPE2_COLUMN) * sizeof(out_type) * BC_SHAPE1_ROW;\
        check_cust_access(RS1, rs1_size); \
        check_cust_access(RS2, rs2_size); \
...
#define check_traps_medwconv_mm(rs1_type, rs2_type, out_type) ({ \
        ...
        check_cust_misaligned_stride_src(RS1, rs1_type, CONV_IN_STRIDE * sizeof(rs1_type)); \
        check_cust_misaligned_stride_src(RS2, rs2_type, CONV_W_STRIDE * sizeof(rs2_type)); \
...
#define check_trap_mov_l1_glb(esize)({ \
        ...
        check_tcp_access_end_l1(RD + ((DMAE_STRIDE_D_X ? DMAE_STRIDE_D_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y + \
          (DMAE_STRIDE_D_Y ? DMAE_STRIDE_D_Y : (DMAE_STRIDE_D_X ? DMAE_STRIDE_D_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y)) * DMAE_SHAPE_Z * esize) \
})

//check trap mov.glb.l1
#define check_trap_mov_glb_l1(esize)({ \
        check_tcp_access_start_l1(RS1) \
        check_tcp_access_end_l1(RS1 + ((DMAE_STRIDE_S_X ? DMAE_STRIDE_S_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y + \
          (DMAE_STRIDE_S_Y ? DMAE_STRIDE_S_Y : (DMAE_STRIDE_S_X ? DMAE_STRIDE_S_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y)) * DMAE_SHAPE_Z * esize) \
})
...
#define check_trap_mov_glb_llb(esize) ({ \
  check_tcp_access_start_llb_mov(RS1) \
  check_tcp_access_end_llb(RS1 + ((DMAE_STRIDE_S_X ? DMAE_STRIDE_S_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y + \
    (DMAE_STRIDE_S_Y ? DMAE_STRIDE_S_Y : (DMAE_STRIDE_S_X ? DMAE_STRIDE_S_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y)) * DMAE_SHAPE_Z * esize) \
})
...
#define check_trap_mov_llb_llb(in_esize, out_esize) ({ \
  check_tcp_access_start_llb_mov(RS1) \
  check_tcp_access_end_llb(RS1 + ((DMAE_STRIDE_S_X ? DMAE_STRIDE_S_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y + \
    (DMAE_STRIDE_S_Y ? DMAE_STRIDE_S_Y : (DMAE_STRIDE_S_X ? DMAE_STRIDE_S_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y)) * DMAE_SHAPE_Z * in_esize) \
  check_tcp_access_start_llb_mov(RD) \
  check_tcp_access_end_llb(RD + ((DMAE_STRIDE_D_X ? DMAE_STRIDE_D_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y + \
    (DMAE_STRIDE_D_Y ? DMAE_STRIDE_D_Y : (DMAE_STRIDE_D_X ? DMAE_STRIDE_D_X : DMAE_SHAPE_X) * DMAE_SHAPE_Y)) * DMAE_SHAPE_Z * out_esize) \
})
