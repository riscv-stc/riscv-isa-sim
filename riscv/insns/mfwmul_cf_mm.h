...
},
{
    if (P.MU.mfp32 == MTYPE_FP32) {
      float32_t &td_w = P.MU.tr_elt<float32_t>(td_num + m, 0, i, j, mmax, des_nmax * amul * wide, reg_rename, true); \
      td_w = f32_mul(f16_to_f32(ts1), f16_to_f32(ts2));
    } else if (P.MU.mfp32 == MTYPE_TFP32){
      tfloat32_t &td_w = P.MU.tr_elt<tfloat32_t>(td_num + m, 0, i, j, mmax, des_nmax * amul * wide, reg_rename, true); \
...
{
    float64_t &td_w = P.MU.tr_elt<float64_t>(td_num + m, 0, i, j, mmax, des_nmax * amul * wide, reg_rename, true);
