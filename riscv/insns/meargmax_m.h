...
//p->run_async([p, t_rs1, t_rs2, t_rd, pc]() {
  ...
  case 0x0: // f16 = uint16_t
    ...
    bc_sst_fill(sst, 2, 2);
    CusIns.veargmax_m((half*)rs1, (uint16_t*)rd, &sst, 1);
    WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_MME_DATA16);
    break;
  case 0x010101: // bf16 = uint16_t
    check_traps_meargxxx_m(int16_t, int16_t);
    bc_sst_fill(sst, 2, 2);
    CusIns.veargmax_m((Bfloat16*)rs1, (uint16_t*)rd, &sst, 1);
    WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_MME_DATA16);
    break;
  case 0x020202: // f32 = uint16_t
    check_traps_meargxxx_m(int32_t, int16_t);
    bc_sst_fill(sst, 4, 2);
    CusIns.veargmax_m((float32_t*)rs1, (uint16_t*)rd, &sst, 1);
    WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_MME_DATA16);
    break;
