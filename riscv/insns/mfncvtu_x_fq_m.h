  ...
  { 
    if (P.MU.mfp32 == MTYPE_FP32){
        ...
        P.MU.acc_elt<uint8_t>(td_num + m, 0, i, j, mmax, nmax * amul * 2, reg_rename, true) = f32_to_ui8(ts1, softfloat_roundingMode, true);
    } else if (P.MU.mfp32 == MTYPE_TFP32) {
        auto ts1 = P.MU.acc_elt<tfloat32_t>(ts1_num + m, 0, i, j, mmax, nmax * amul, false, false);
        P.MU.acc_elt<uint8_t>(td_num + m, 0, i, j, mmax, nmax * amul * 2, reg_rename, true) = tf32_to_ui8(ts1, softfloat_roundingMode, true);
    } else {
        require(0);
    }
  },
  {
    ...
    P.MU.acc_elt<uint8_t>(td_num + m, 0, i, j, mmax, nmax * amul * 2, reg_rename, true) = f64_to_ui16(ts1, softfloat_roundingMode, true);
  },
  {
    ;
  },
