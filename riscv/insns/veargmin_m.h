...
switch (VME_DTYPE) {
  case 0x0:         // f16
    if (DIM == 0) {
        ...
        uint32_t res;
        CusIns.veargmin_m((half*)rs1, &res, &sst);
        WRITE_RD_STC(res);
    } else {
        check_traps_vexxx_m_reduce_vector(uint16_t);
        CusIns.veargmin_m((half*)rs1, (uint16_t*)rd, &sst, DIM_DM);
        if(DIM_DM){
  ...
  case 0x01:     // bf16
    if (DIM == 0) {
        check_traps_vexxx_m_reduce_all(uint16_t);
        uint32_t res;
        CusIns.veargmin_m((Bfloat16*)rs1, &res, &sst);
        WRITE_RD_STC(res);
    } else {
        check_traps_vexxx_m_reduce_vector(uint16_t);
        CusIns.veargmin_m((Bfloat16*)rs1, (uint16_t*)rd, &sst, DIM_DM);
        if(DIM_DM){
            WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_VME_COM_W);
        } else {
            WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_VME_COM_H);
  ...
  case 0x02:    // f32
    if (DIM == 0) {
        ...
        uint32_t res;
        CusIns.veargmin_m((float32_t*)rs1, &res, &sst);
        WRITE_RD_STC(res);
    } else {
        check_traps_vexxx_m_reduce_vector(uint32_t);
        CusIns.veargmin_m((float32_t*)rs1, (uint16_t*)rd, &sst, DIM_DM);
        if(DIM_DM){
            WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_VME_COM_W);
        } else {
            WRITE_MEM_STC(RD, (uint16_t*)rd, CMT_LOG_VME_COM_H);
