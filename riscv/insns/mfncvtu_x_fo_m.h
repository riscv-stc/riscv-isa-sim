  ...
  { 
    if (P.MU.mfp32 == MTYPE_FP32){
        ...
        auto temp = tf32_to_i4(ts1, softfloat_roundingMode, true);
        auto &accd = P.MU.acc_elt<uint8_t>(td_num + m, 0, i, j / 2 , mmax, nmax * amul * 2, reg_rename, true);
        accd = j % 2 ? ((temp & 0xF) << 4) | accd : temp & 0xF;
    } else {
        require(0);
  ...
  },
  {
    ...
    P.MU.acc_elt<uint8_t>(td_num + m, 0, i, j, mmax, nmax * amul * 2, reg_rename, true) = f64_to_i8(ts1, softfloat_roundingMode, true);
  },
  {
    ;
  },
