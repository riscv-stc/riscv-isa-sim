  ...
  },
  {
    ...
    case MTYPE_FP8E4M3:
      if (P.MU.mfp16 == MTYPE_FP16){
        ...
        P.MU.acc_elt<float8_e4m3_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = f16_to_f8e4m3(rs1);
      } else if (P.MU.mfp16 == MTYPE_BF16){
        auto rs1 = P.MU.acc_elt<bfloat16_t>(ts1_num + m , 0, i, j, mmax, nmax * amul, false, false);
        P.MU.acc_elt<float8_e4m3_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = bf16_to_f8e4m3(rs1);
      } else {
        require(0);
      }
      break;
    case MTYPE_FP8E5M2:
      if (P.MU.mfp16 == MTYPE_FP16){
        auto rs1 = P.MU.acc_elt<float16_t>(ts1_num + m , 0, i, j, mmax, nmax * amul, false, false);
        P.MU.acc_elt<float8_e5m2_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = f16_to_f8e5m2(rs1);
      } else if (P.MU.mfp16 == MTYPE_BF16){
        auto rs1 = P.MU.acc_elt<bfloat16_t>(ts1_num + m , 0, i, j, mmax, nmax * amul, false, false);
        P.MU.acc_elt<float8_e5m2_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = bf16_to_f8e5m2(rs1);
      } else {
        require(0);
      }
      break;
    case MTYPE_FP8E3M4:
      if (P.MU.mfp16 == MTYPE_FP16){
        auto rs1 = P.MU.acc_elt<float16_t>(ts1_num + m , 0, i, j, mmax, nmax * amul, false, false);
        P.MU.acc_elt<float8_e3m4_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = f16_to_f8e3m4(rs1);
      } else if (P.MU.mfp16 == MTYPE_BF16){
        auto rs1 = P.MU.acc_elt<bfloat16_t>(ts1_num + m, 0, i, j, mmax, nmax * amul, false, false);
        P.MU.acc_elt<float8_e3m4_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = bf16_to_f8e3m4(rs1);
      } else {
        require(0);
      }
    default:
  ...
  },
  {
    ...
    if (P.MU.mfp32 == MTYPE_FP32) {
      auto ts1 = P.MU.acc_elt<float64_t>(ts1_num + m, 0, i, j, mmax, nmax, false, false);
      P.MU.acc_elt<float32_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = f64_to_f32(ts1);
    } else if (P.MU.mfp32 == MTYPE_TFP32) {
      auto rs1 = P.MU.acc_elt<float64_t>(ts1_num + m , 0, i, j, mmax, nmax * amul, false, false);
      P.MU.acc_elt<tfloat32_t>(td_num + m, 0, i, j, mmax, des_nmax * amul, reg_rename, true) = f64_to_tf32(rs1);
    } else {
      require(0);
    }
