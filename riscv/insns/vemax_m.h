...
switch (VME_DTYPE) {
  case 0x0:         // f16
    if (DIM == 0) {
        ...
        half res;
        CusIns.vemax_m((half*)rs1, &res, &sst);
        float32_t f32res;
  ...
  case 0x10101:     // bf16
    if (DIM == 0) {
        check_traps_vexxx_m_reduce_all(uint16_t);
        Bfloat16 res;
        CusIns.vemax_m((Bfloat16*)rs1, &res, &sst);
        float32_t f32res;
        f32res.v = Float32(res).x;
        WRITE_FRD_STC(f32res);
    } else {
        check_traps_vexxx_m_reduce_vector(uint16_t);
        CusIns.vemax_m((Bfloat16*)rs1, (Bfloat16*)rd, &sst, DIM_DM);
        if(DIM_DM){
  ...
  case 0x020202:    // f32
    if (DIM == 0) {
        ...
        float32_t res;
        CusIns.vemax_m((float32_t*)rs1, &res, &sst);
        //float32_t f32res;
        //f32res.v = Float32(res).x;
        WRITE_FRD_STC(res);
    } else {
        check_traps_vexxx_m_reduce_vector(uint32_t);
        CusIns.vemax_m((float32_t*)rs1, (float32_t*)rd, &sst, DIM_DM);
        if(DIM_DM){
