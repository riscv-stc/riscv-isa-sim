  MXU_MFP_CVT_SCALE
  ({
    auto rs1 = P.MU.acc_elt<uint8_t>(ts1_num + m , 0, i, j, mmax, nmax * amul, false, false);
    switch (P.MU.mfp8)
    {
    case MTYPE_FP8E4M3:
      P.MU.acc_elt<float8_e4m3_t>(td_num + m, 0, i, j, mmax, nmax * amul, reg_rename, true) = ui8_to_f8e4m3(rs1);
      break;
    case MTYPE_FP8E5M2:
      P.MU.acc_elt<float8_e5m2_t>(td_num + m, 0, i, j, mmax, nmax * amul, reg_rename, true) = ui8_to_f8e5m2(rs1);
      break;
    case MTYPE_FP8E3M4:
      P.MU.acc_elt<float8_e3m4_t>(td_num + m, 0, i, j, mmax, nmax * amul, reg_rename, true) = ui8_to_f8e3m4(rs1);
      break;
    default:
      require(0);
      break;
  ...
  1, {require(P.MU.msew >= e8 && P.MU.msew <= 64);}, P.MU.msew)
