...
switch (VME_DTYPE) {
  case 0x0:         // f16
    ...
    CusIns.vemin_mm((half*)rs1, (half*)rs2, (half*)rd, &sst);
    WRITE_MEM_STC(RD, (half*)rd, CMT_LOG_VME_COM);
    break;
  case 0x01:     // bf16
    check_traps_vexxx_mm(uint16_t);
    CusIns.vemin_mm((Bfloat16*)rs1, (Bfloat16*)rs2, (Bfloat16*)rd, &sst);
    WRITE_MEM_STC(RD, (Bfloat16*)rd, CMT_LOG_VME_COM);
    break;
  case 0x02:    // f32
    check_traps_vexxx_mm(uint32_t);
    CusIns.vemin_mm((float32_t*)rs1, (float32_t*)rs2, (float32_t*)rd, &sst);
...
//     vemin_mm<dtype_vd>((dtype_vd*)rs1, (dtype_vd*)rd, (dtype_vd*)rs2, &sst, relu);
