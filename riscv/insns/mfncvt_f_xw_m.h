  ...
  },
  {
    ...
    if (P.MU.mfp8 == MTYPE_FP8E4M3){
      P.MU.acc_elt<float8_e4m3_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = i16_to_f8e4m3(ts1);
    } else if (P.MU.mfp8 == MTYPE_FP8E5M2) {
      P.MU.acc_elt<float8_e5m2_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = i16_to_f8e5m2(ts1);
    } else if (P.MU.mfp8 == MTYPE_FP8E3M4) {
      P.MU.acc_elt<float8_e3m4_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = i16_to_f8e3m4(ts1);
    } else {
      require(0);
    }
  },
  ...
  },
  {
    ...
    if (P.MU.mfp32 == MTYPE_FP32) {
      ...
      P.MU.acc_elt<tfloat32_t>(td_num + m, 0, i, j, mmax, nmax * 2 * amul, reg_rename, true) = i64_to_tf32(ts1);
    } else {
      require(0);
    }
  },
