...
struct : public arg_t {
  std::string to_string(insn_t insn) const {
	return vediv_name[insn.vediv()];
  }
} vediv;

struct : public arg_t {
  std::string to_string(insn_t insn) const {
	return vlmul_name[insn.vlmul()];
  }
} vlmul;

struct : public arg_t {
  std::string to_string(insn_t insn) const {
	return vsew_name[insn.vsew()];
  }
} vsew;
...
disassembler_t::disassembler_t(int xlen)
{
  ...
  #define DEFINE_SV4TYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vrs2, &frs1, &vm})
  #define DEFINE_SV5TYPE(code) DISASM_INSN(#code, code, 0, {&xrd, &vrs2, &xrs1})
  ...
  #define DEFINE_SV17TYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vrs2, &vrs1})
  #define DEFINE_SV18TYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &xrs1})
  #define DEFINE_SV19TYPE(code) DISASM_INSN(#code, code, 0, {&xrd, &vrs2, &vm})
  #define DEFINE_SV20TYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &frs1})
  #define DEFINE_SV21TYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vm})
  ...
  DEFINE_SV14TYPE(vfmsac_vf);
  DEFINE_SV13TYPE(vfmsac_vv);
  DEFINE_SV14TYPE(vfnmsac_vf);
  DEFINE_SV13TYPE(vfnmsac_vv);
  ...
  DEFINE_SV14TYPE(vfnmsub_vf);
  DEFINE_SV13TYPE(vfnmsub_vv);
  ...
  DEFINE_SV7TYPE(vmfeq_vf);
  DEFINE_SV6TYPE(vmfeq_vv);
  DEFINE_SV7TYPE(vmfne_vf);
  DEFINE_SV6TYPE(vmfne_vv);
  DEFINE_SV7TYPE(vmflt_vf);
  DEFINE_SV6TYPE(vmflt_vv);
  DEFINE_SV7TYPE(vmfgt_vf);
  DEFINE_SV6TYPE(vmfgt_vv);
  DEFINE_SV7TYPE(vmfle_vf);
  DEFINE_SV6TYPE(vmfle_vv);
  DEFINE_SV7TYPE(vmfge_vf);
  DEFINE_SV6TYPE(vmfge_vv);
  DEFINE_SV15TYPE(vfmv_f_s);
  DEFINE_SV20TYPE(vfmv_s_f);
  ...
  DEFINE_SV6TYPE(vfredmax_vs);
  DEFINE_SV6TYPE(vfredmin_vs);
  DEFINE_SV6TYPE(vfredosum_vs);
  DEFINE_SV6TYPE(vfredsum_vs);
