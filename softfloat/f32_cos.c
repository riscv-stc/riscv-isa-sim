...
float32_t f32_cos( float32_t a )
{
  ...
  if( a.v > 0xc0490fda || a.v < 0x80000000 )
  {
    signA = 0;
    ...
    //if a is 2^k to 2^k-23, use 2^-k+22 to 2^-k-9, 2^-k-10 to 2^-k-41 bits to compute a/(2pi), bigger than these bits won't produce decimal bits
