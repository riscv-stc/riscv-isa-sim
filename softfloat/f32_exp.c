...
float32_t f32_exp( float32_t a )
{
  ...
  float32_t ln2 = { 0x3f317218 } ; //0.69314718055994530941723212145818
  float32_t recip_ln2 = { 0x3fb8aa3b } ; //1.4426950408889634073599246810019
  float32_t P1 = { 0x3c088889 }; // 1/120
  float32_t P2 = { 0x3d2aaaab }; // 1/24
  float32_t P3 = { 0x3e2aaaab }; // 1/6
  float32_t P4 = { 0x3f000000 }; // 1/2
  float32_t P5 = { 0x3f800000 }; // 1/1
   ...
   */ 
  x = f32_mul( a, recip_ln2 );
  k = f32_roundToInt( x, softfloat_round_near_maxMag, false );
  r = f32_mul( k, ln2 ); 
  r = f32_sub( a, r );

  //compute e^r using taylor series
  r = f32_add( P5, f32_mul( r, 
  f32_add( P5, f32_mul( r,
  f32_add( P4, f32_mul( r, 
  f32_add( P3, f32_mul( r, P2 ) ) ) ) ) ) ) ); 

  int_fast32_t expz = f32_to_i32( k, softfloat_round_near_maxMag, false ) + ( ( ( int_fast32_t )( r.v & 0x7f800000 ) ) >> 23 );
  if( expz < -23 )
  {
    uZ.ui = 0;
    return uZ.f;
  ...
  { //subnormal number
    uZ.ui = ( ( r.v & 0x7FFFFF ) + 0x800000 ) >> ( -expz + 1 );
    
    // round the truncation bits
    if( ( ( ( r.v & 0x7FFFFF ) + 0x800000 ) & ( ( 1 << (-expz+1) ) - 1 ) )  >=  ( 1 << (-expz) )  )
    {
      uZ.ui += 1;
    }
    return uZ.f;
