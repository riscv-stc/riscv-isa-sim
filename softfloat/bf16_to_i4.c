...
int_fast8_t bf16_to_i4( bfloat16_t a, uint_fast8_t roundingMode, bool exact )
{
  union ui16_bf16 uA;
  uint_fast16_t uiA;
  bool sign;
  ...
  if ( 0xFF == exp )
  {
    ...
    return frac ? i4_fromNaN 
                : sign ? i4_fromNegOverflow : i4_fromPosOverflow;
  }
  ...
  if( exp )
  {
    ...
    if( 0 <= shiftDist )
    {
      ...
      if( ( 0x8 == sig32 ) && ( 1 == sign ) && ( 0 == shiftDist ) )
      {//if -128
        return -16;
      }
      else
      {//overflow
        softfloat_raiseFlags( softfloat_flag_invalid );
        return sign ? i4_fromNegOverflow : i4_fromPosOverflow;        
      }
